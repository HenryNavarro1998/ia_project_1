...
def evaluate(board):
    ...
    return red_count - blue_count # Valor positivo favorece a "x"
...
def minimax(board, depth, alpha, beta, player):
    ...
    best_move = None
    if player:  # Maximizar (jugador "o")
        max_eval = float("-inf")
        moves = sorted(generate_moves(board, "o"), key=get_sort_move_key, reverse=True)
        for move in moves:
    ...
    else:  # Minimizar (jugador "x")
        min_eval = float("inf")
        moves = sorted(generate_moves(board, "x"), key=get_sort_move_key)
        for move in moves:
            new_board = handle_move(board, move)
